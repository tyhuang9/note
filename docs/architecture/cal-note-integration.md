...
| 8 — Cal import and unified backups | **Accepted** | Main-only read-only Cal preview/import with fixed duplicate policy and recovery snapshot, plus versioned secret-free unified backup/restore with staged validation, durable recovery journal, and fail-closed reconciliation. |
| 9 — Release hardening | **Accepted** | Privacy-safe bounded local timing diagnostics, deterministic synthetic calendar reference samples, explicit CI quality checks, release/platform evidence, and final accessibility/security/QA review with documented host and runtime limits. |
...
Phase 8 is **accepted complete**. Cal import is explicit and main-window-only: the native dialog selects a regular, canonical source database, which is then opened immutable/read-only. Preview validates bounded compatible source content, schema/integrity/foreign keys and a source fingerprint; it returns only a basename and bounded counts/items. The only duplicate behavior is `skipExisting`, backed by stored import provenance. A preview is identity-bound, expiring, and one-use; commit rechecks it, creates a verified recovery snapshot before mutation, imports in one destination transaction, and verifies result counts. Sources containing unimplemented multi-calendar, reminder, or occurrence-override data fail closed rather than silently losing records.

Unified backups use a versioned `note-unified-backup-v1` archive built by allowlist: validated Note data, a verified calendar snapshot, and only explicit non-secret preferences/asset metadata. The format excludes credentials, provider/model data, model weights, recordings, and arbitrary application files by construction. Archive preview rejects unexpected/duplicate/traversal-style entries, malformed manifests, checksum/version mismatches, and bounded-size violations before an expiring one-use restore session exists. Restore creates a verified recovery archive and records a private fsynced intent before publication. A process guard blocks Note and calendar mutations through all post-intent failures; startup reconciles an unfinished restore from the verified recovery archive before services become available, or fails closed with a sanitized recovery-required state.

Final Phase 8 evidence: Rust format/check/strict Clippy and 271 library tests passed; the production frontend build and focused migration E2E 3/3 passed; security, UX, final accessibility, QA, and final integration reviews were GO; diff checks passed. CodeRabbit could not run because its CLI was absent and its supported installer rejected this Windows/MSYS host, so no CodeRabbit result is claimed. Full Calendar/browser aggregate commands timed out with EPIPE before producing a test-level result. Manual packaged-desktop validation remains needed for native dialogs/capabilities, disposable real-Cal schema variants, crash/power-loss recovery on supported filesystems, and screen-reader behavior. Cal remained exactly at its recorded reference SHA and raw porcelain digest.

## Phase 9 implementation record

Phase 9 is **accepted complete**. Native timing uses a private fixed operation enum, duration-only monotonic timers, and an optional bounded local stderr diagnostic layer. The layer accepts only the private operation field, emits only a static operation label, elapsed milliseconds, and a fixed outcome, and drops diagnostics when its fixed 128-entry channel is full. It is not telemetry: it has no network/export/file sink and never accepts user content, identifiers, dates, paths, URLs, provider names, error text, credentials, or record counts. If another global tracing subscriber already exists, the local layer is intentionally unavailable rather than replacing it. Browser timing uses fixed literal operation names with unique ephemeral User Timing entries that are cleared after each resolved or rejected operation; no observer or retained browser metrics are claimed.

The release record includes a deterministic temporary SQLite reference harness with 1,000 generic records. On this Windows debug-test host, 11 samples produced list p50/p95 22/23 ms, search 5/6 ms, and create/delete CRUD 0/0 ms. These are local synthetic reference results, not release targets, performance guarantees, or cross-platform measurements. Direct native timer-to-layer and concurrent/rejected browser cleanup tests protect the instrumentation behavior. The CI release workflow now explicitly runs formatting, strict linting, locked Rust tests, and frontend build before packaging on its Windows, macOS, and Linux matrix.

Final Phase 9 evidence: Rust format/check/strict Clippy and 276 tests passed; production frontend build with CSP/chunk isolation and the focused browser performance helper test 1/1 passed; a serial Chromium regression baseline passed 146/152, with the six failures limited to the known July-2026 fixed-date calendar fixture expectations. The latest complete `npm run test:e2e` attempt reached its 240-second host cap and produced reporter EPIPE without a test-level result, so it is unverified rather than pass/fail. Security, observability, code-quality, UX, final accessibility, QA, and final integration reviews were GO. CodeRabbit remained unavailable on this Windows/MSYS host and is not represented as completed. Packaged desktop dialogs/capabilities, physical notification/shortcut/microphone/model behavior, first-frame/first-ready and canvas-FPS comparison, screen-reader assistive-technology runs, filesystem power-loss recovery, and macOS/Linux X11/Wayland release-runtime behavior remain manual or platform validation work. Cal remained exactly at its recorded reference SHA and raw porcelain digest.
