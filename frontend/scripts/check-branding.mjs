...
const [appCss, appHtml, docsCss, docsHome, docsInstall, readme, titleBar, aiProviders, tauriConfigText, mark, mark32, mark48, markDark32, markDark48, windowsIcon, macosIcon, linuxIcon] = await Promise.all([
  ...
  readFile(new URL("../../backend/src-tauri/tauri.conf.json", import.meta.url), "utf8"),
  ...
  readFile(new URL("../../docs/assets/note-mark-dark-32.png", import.meta.url)),
  readFile(new URL("../../docs/assets/note-mark-dark-48.png", import.meta.url)),
  readFile(new URL("../../backend/src-tauri/icons/icon.ico", import.meta.url)),
  readFile(new URL("../../backend/src-tauri/icons/icon.icns", import.meta.url)),
  readFile(new URL("../../backend/src-tauri/icons/128x128.png", import.meta.url)),
]);
...
for (const [name, derivative, expectedSize] of [["32px mark", mark32, 32], ["48px mark", mark48, 48], ["dark 32px mark", markDark32, 32], ["dark 48px mark", markDark48, 48]]) {
...
assert.match(appHtml, /data-note-favicon="app"/);
assert.match(appHtml, /theme-color/);
assert.match(titleBar, /note-mark-32\.png/);
assert.match(titleBar, /note-mark-dark-32\.png/);
assert.match(titleBar, /data-theme-variant/);
assert.match(titleBar, /data-note-favicon/);
assert.match(docsHome, /note-mark-48\.png/);
assert.match(docsInstall, /note-mark-48\.png/);
assert.match(docsHome, /note-mark-dark-48\.png/);
assert.match(docsInstall, /note-mark-dark-48\.png/);
...
assert.doesNotMatch(appCss, /box-shadow:\s*inset\s+[1-9]\d*px\s+0(?:\s+0)?/i, "selected controls must not use colored left-edge markers");
assert.doesNotMatch(appCss, /border-left:\s*[^;]*(?:brand-gold|#D4A128|#E6B84A|#7A5700)/i, "app surfaces must not use brand-colored left borders");
assert.match(appCss, /@media \(hover: none\), \(pointer: coarse\)[\s\S]*\.bookmark-toggle/, "touch and hybrid-pointer devices must keep explorer actions available");
assert.match(docsCss, /nav a\s*{[^}]*min-height:\s*44px/s, "mobile docs navigation must keep accessible touch targets");
const tauriConfig = JSON.parse(tauriConfigText);
assert.equal(tauriConfig.app.windows[0].skipTaskbar, false, "the primary desktop window must remain visible in the taskbar");
assert.deepEqual(tauriConfig.bundle.icon, [
  ...
  "icons/icon.icns",
  "icons/icon.ico",
], "desktop packaging must reference Linux PNG, macOS ICNS, and Windows ICO icons");
assert.deepEqual([...windowsIcon.subarray(0, 4)], [0, 0, 1, 0], "Windows bundle icon must be an ICO file");
assert.equal(macosIcon.subarray(0, 4).toString("ascii"), "icns", "macOS bundle icon must be an ICNS file");
assert.deepEqual([...linuxIcon.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10], "Linux bundle icon must be a PNG file");
assert.ok(frontendRoot.endsWith("frontend\\") || frontendRoot.endsWith("frontend/"));

console.log("Branding assets, theme variants, desktop package icons, palette, metadata, and messaging are connected.");
