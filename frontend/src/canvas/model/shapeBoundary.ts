...
function closestEllipsePoint(point: CanvasPoint, center: CanvasPoint, radiusX: number, radiusY: number): CanvasPoint | null { if (radiusX === 0 || radiusY === 0) return { x: center.x, y: center.y }; const dx = point.x - center.x; const dy = point.y - center.y; let best: CanvasPoint | null = null; for (const seed of [Math.atan2(dy * radiusX, dx * radiusY), 0, Math.PI / 2, Math.PI, -Math.PI / 2]) { let angle = seed; for (let i = 0; i < 24; i += 1) { const sin = Math.sin(angle); const cos = Math.cos(angle); const f = (radiusY * radiusY - radiusX * radiusX) * sin * cos + radiusX * dx * sin - radiusY * dy * cos; const derivative = (radiusY * radiusY - radiusX * radiusX) * (cos * cos - sin * sin) + radiusX * dx * cos + radiusY * dy * sin; if (Math.abs(derivative) < 1e-12) break; angle -= f / derivative; } for (const candidateAngle of [angle, angle + Math.PI]) { const candidate = { x: center.x + radiusX * Math.cos(candidateAngle), y: center.y + radiusY * Math.sin(candidateAngle) }; if (!best || distanceSquared(point, candidate) < distanceSquared(point, best)) best = candidate; } } return best; }
